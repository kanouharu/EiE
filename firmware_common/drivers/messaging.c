...
MessageSlotType: member of the message queue holding the free/full status of a particular queue location and a pointer to its data

MessageSlotType: token, state and timestamp of a message in the queue
...
to queue messages.  The message queue is a finite resource with U8_TX_QUEUE_SIZE slots available for messages.
...
static MessageSlotType Msg_asPool[U8_TX_QUEUE_SIZE];              /* Array of MessageSlotType used for the transmit queue */
...
it has been sent. */
static MessageStatusType Msg_asStatusQueue[U8_STATUS_QUEUE_SIZE]; /* Array of MessageSlotType used to monitor message status */
static MessageSlotType* Msg_pNextStatus;                   /* Pointer to next available message status */
